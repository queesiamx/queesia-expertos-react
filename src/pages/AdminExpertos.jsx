...
// (si no usas emailjs aquí, puedes borrar este import)
...
export default function AdminExpertos() {
  ...
  // ✅ Usa el contexto global (no listeners locales)
  const { user, loading, rol } = useAuth();

  // ✅ Verificación de acceso (una sola vez por cambio de user/rol)
  useEffect(() => {
    if (loading) return; // espera auth
    setVerificado(true);
  ...
  // ✅ Carga de expertos (solo si autorizado)
  useEffect(() => {
  ...
  if (!autorizado) {
    return (
      <div className="p-8 text-center text-red-600 font-bold">
        ⚠️ Acceso restringido. Esta vista es solo para administradores
        autorizados.
      </div>
    );
  }
  ...
  const totalServicios = expertos.reduce(
    (acc, exp) => acc + (Array.isArray(exp.servicios) ? exp.servicios.length : 0),
    0
  );
