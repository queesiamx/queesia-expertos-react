// src/components/ConsultaBox.jsx
import { useState, useMemo } from "react";
import { addDoc, collection, serverTimestamp } from "firebase/firestore";
import { db } from "@/firebase";                // ajusta la ruta si no usas alias @
...
export default function ConsultaBox({
  ...
  // --- LOGIN rápido con Google (o redirige a /login si prefieres) ---
  ...
  const handleSend = async () => {
    ...
    try {
      await addDoc(collection(db, "consultasModeradas"), {
        tipo: "consulta",
        estado: "porRevisar",           // Admin revisa y sugiere costo
        origen: "publico",
        pregunta: texto.trim(),
        userId: user.uid,
...
if (!user) {
  return (
    <div className={"rounded-2xl border border-slate-200/70 bg-slate-50 p-5 shadow-sm " + className}>
      ...
      >
        {/* Ícono de Google (SVG multicolor) */}
      ...
      {/* ← Se eliminó el botón “Usar otro método” */}
    </div>
  );
}
// ========================================================
  ...
  // Con sesión: textarea habilitado + Enviar
  return (
    <div
      className={
        "rounded-2xl border border-slate-200/70 bg-slate-50 p-5 shadow-sm " +
        className
      }
    >
      ...
      <p className="text-sm text-slate-600 mb-3">
        Antes de enviar tu consulta, el equipo de Quesia la revisará. Algunas
        preguntas podrían requerir una respuesta profesional con costo.
      ...
      <textarea
        ...
        placeholder="Describe tu consulta (sé concreto y aporta contexto)…"
        rows={4}
      ...
      <div className="mt-2 flex items-center justify-between">
        <span
          className={
            "text-xs " + (remaining < 0 ? "text-red-600" : "text-slate-500")
          }
        >
      ...
      <div className="mt-3 text-xs text-slate-500">
        Tras la revisión, el experto verá tu consulta en su panel y podrá
        responder. Si el coste sugerido no concuerda con el suyo, solicitará un
        ajuste a administración antes de publicar.
      </div>
    </div>
  );
}
